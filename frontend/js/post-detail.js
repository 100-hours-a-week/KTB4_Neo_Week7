...
async function fetchReportTypes() {
  const response = await fetch(`${API_BASE_URL}/reports/types`, {
    headers: {
      Authorization: `Bearer ${requireLogin()}`,
    },
  });
...
commentForm?.addEventListener("submit", async (event) => {
  ...
  try {
    if (editingTarget) {
      const response = await fetch(`${API_BASE_URL}/comments/${editingTarget.commentId}`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${requireLogin()}`,
          "Content-Type": "application/json",
      ...
      const response = await fetch(`${API_BASE_URL}/comments/${replyingTarget.parentCommentId}/replies`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${requireLogin()}`,
          "Content-Type": "application/json",
...
commentList?.addEventListener("click", async (event) => {
  ...
  if (commentDeleteButton || replyDeleteButton) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/comments/${targetItem.dataset.commentId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${requireLogin()}`,
        },
